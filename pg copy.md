...
console.log(generator.next()); // Output: { value: "three", done: false }
console.log(generator.next()); // Output: { value: undefined, done: true }
zone.run() - Runs a function inside a Zone.

zone.fork() - Creates a new Zone with a new set of rules and returns it.
```
...
myZone.run(() => {
  console.log(Zone.current.name); // Output: "myZone"
  console.log(Zone.current.get('myProperty')); // Output: "myValue"
});
...
```
zone.get() - Returns the value of a property defined in the Zone.
...
myZone.run(() => {
  console.log(myZone.get('myProperty')); // Output: "myValue"
});
zoneSpec.name - Gets or sets the name of the Zone.

zoneSpec.onFork() - Called when a new Zone is created using zone.fork().

zoneSpec.onIntercept() - Called when a function is intercepted by zone.run().

zoneSpec.onInvoke() - Called when a function is invoked inside a Zone.

zoneSpec.onScheduleTask() - Called when a task is scheduled using zone.scheduleTask().

zoneSpec.onInvokeTask() - Called when a task is invoked inside a Zone.

zoneSpec.onCancelTask() - Called when a task is canceled using zone.cancelTask().

zoneSpec.onHasTask() - Called when zone.hasTask() is called.

zoneSpec.onHandleError() - Called when an error occurs inside a Zone.

zoneSpec.properties - Gets or sets the properties defined in the Zone.
...
zoneSpec.propertyDidChange() - Called when a property defined in the Zone changes.

zoneSpec.propertyWillChange() - Called before a property defined in the Zone changes.
